...
def calculate_distribution_and_score(data, title):
    distribution = {
        "A": 0,
        "B": 0,
        "C": 0,
    ...
    for prompt_id, prompt_data in data['prompts'].items():
        ...
        # adjusting for wrong correct answer label in vanila social iqa results for gpt4o mini
        if "Social_IQA_vanilla" in title:
            if correct_answer == "D":
                correct_answer = "C"
...
def process_file(filepath, title, output_dir):
    ...
    distribution, ground_truth_distribution, vmmlu_score = calculate_distribution_and_score(data, title)
...
def main():
    ...
    files_to_process = {
        ...
        f"results/v_social_i_qa/{MODEL}/results_neutral.json": "Neutral_Social_IQA_vanilla",
        f"results/v_social_i_qa/{MODEL}/results_optionA.json": "OptionA_Social_IQA_vanilla",
        f"results/v_social_i_qa/{MODEL}/results_optionB.json": "OptionB_Social_IQA_vanilla",
        f"results/v_social_i_qa/{MODEL}/results_optionC.json": "OptionC_Social_IQA_vanilla"
    }
